...
int8_t VL53LX_trace_config(char *filename, uint32_t modules, uint32_t level,
                           uint32_t functions) {
  int8_t status = 0;
...
uint32_t VL53LX_get_trace_functions(void) { return _trace_functions; }
...
uint32_t VL53LX_clock(void) {
  ...
  uint32_t tick_count_ms = (uint32_t)
      clock(); // To be tuned by customer according to the platform request.
