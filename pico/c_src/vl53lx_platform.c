...
void Robocup_PlatformInit(Robocup_Platform_t *platform) {
  robocup_platform_instance = platform;
}
...
VL53LX_Error VL53LX_WriteMulti(VL53LX_Dev_t *pdev, uint16_t index,
  ...
  if (!robocup_platform_instance) {
    return VL53LX_ERROR_NOT_IMPLEMENTED;
  }

  return robocup_platform_instance->writeMulti(pdev->i2c_slave_address, index,
                                               pdata, count);
}
...
VL53LX_Error VL53LX_ReadMulti(VL53LX_Dev_t *pdev, uint16_t index,
  ...
  return robocup_platform_instance->readMulti(pdev->i2c_slave_address, index,
                                              pdata, count);
}
...
VL53LX_Error VL53LX_RdWord(VL53LX_Dev_t *pdev, uint16_t index,
                           uint16_t *pdata) {
  VL53LX_Error status = VL53LX_ERROR_NONE;
  uint8_t buffer[2];
...
VL53LX_Error VL53LX_WaitUs(VL53LX_Dev_t *pdev, int32_t wait_us) {
  ...
  robocup_platform_instance->waitUs(wait_us);
  return 0;
}
...
VL53LX_Error VL53LX_WaitMs(VL53LX_Dev_t *pdev, int32_t wait_ms) {
  ...
  robocup_platform_instance->waitMs(wait_ms);
  return 0;
}
...
VL53LX_Error VL53LX_GetTickCount(VL53LX_Dev_t *pdev, uint32_t *ptick_count_ms) {
  ...
  *ptick_count_ms = robocup_platform_instance->getTickCount();

  /* To be filled by customer according to the platform request. Return 0 if OK
   */
  /* example: *ptick_count_ms = timeGetTime();   */
  trace_print(VL53LX_TRACE_LEVEL_DEBUG, "VL53LX_GetTickCount() = %5u ms;\n",
              *ptick_count_ms);
  return 0;
}
